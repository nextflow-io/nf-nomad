...
import nextflow.k8s.model.*
import nextflow.nomad.model.PodSpecBuilder
...
class PodSpecBuilder {
    ...
    List<nextflow.k8s.model.PodEnv> envVars = []
    ...
    Collection<nextflow.k8s.model.PodMountEmptyDir> emptyDirs = []

    Collection<nextflow.k8s.model.PodMountSecret> secrets = []

    Collection<PodHostMount> hostMounts = []

    Collection<nextflow.k8s.model.PodVolumeClaim> volumeClaims = []
    ...
    PodSpecBuilder withEnv(nextflow.k8s.model.PodEnv env ) {
        envVars.add(env)
        return this
    }
    ...
    PodSpecBuilder withVolumeClaim(nextflow.k8s.model.PodVolumeClaim claim ) {
        volumeClaims.add(claim)
        return this
    }

    PodSpecBuilder withVolumeClaims( Collection<nextflow.k8s.model.PodVolumeClaim> claims ) {
    ...
    PodSpecBuilder withEmptyDirs( Collection<nextflow.k8s.model.PodMountEmptyDir> emptyDirs ) {
        this.emptyDirs.addAll(emptyDirs)
        return this
    }

    PodSpecBuilder withEmptyDir(nextflow.k8s.model.PodMountEmptyDir emptyDir ) {
        this.emptyDirs.add(emptyDir)
        return this
    } 

    PodSpecBuilder withSecrets( Collection<nextflow.k8s.model.PodMountSecret> secrets ) {
        this.secrets.addAll(secrets)
        return this
    }

    PodSpecBuilder withSecret(nextflow.k8s.model.PodMountSecret secret ) {
    ...
    PodSpecBuilder withPodOptions(nextflow.k8s.model.PodOptions opts) {
        // -- pull policy
    ...
    Map build() {
        ...
        final env = []
        for( nextflow.k8s.model.PodEnv entry : this.envVars ) {
        ...
        for( nextflow.k8s.model.PodVolumeClaim entry : volumeClaims ) {
        ...
        // -- emptyDir volumes
        for( nextflow.k8s.model.PodMountEmptyDir entry : emptyDirs ) {
            final name = nextVolName()
        ...
        // -- secret volumes
        for( nextflow.k8s.model.PodMountSecret entry : secrets ) {
            final name = nextVolName()
    ...
    @PackageScope
    @CompileDynamic
    static void secretToSpec(String volName, nextflow.k8s.model.PodMountSecret entry, List mounts, List volumes ) {
        assert entry
