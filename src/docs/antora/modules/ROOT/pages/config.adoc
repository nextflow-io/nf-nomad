...
- `enabled`: allow enabling/disable initial validations of the plugin
...
- `nodePool`: The Nomad node pool to assign to all Nextflow jobs. Assigns jobs to a specific pool of nodes for resource isolation or workload segmentation. Takes precedence over any `node.pool` constraint used for the same purpose. Can also be set via the `NOMAD_NODE_POOL` environment variable.
...
| `pool = '...'`
| string
| `${node.pool} = <value>` — filters placement to nodes in the pool, but does not assign the job to the pool. Prefer `nomad.jobs.nodePool` (or `nomadOptions.nodePool`) for first-class pool assignment.
...
- `nodePool`: string — assigns the job to a specific Nomad node pool, overriding any global `nomad.jobs.nodePool` for this process
